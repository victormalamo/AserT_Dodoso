...
////////////////////// ESSENTIAL ITEMS ////////////////////////////////////// 
	...
	$scope.show_asteric = '';
	$scope.assessment_tented_camp_form_changes = false;
	$scope.show_asteric_tented_camp = '';
	$scope.assessment_tented_camp_form_changes = false;
	$scope.show_asteric_tented_camp = '';
	$scope.assessment_tented_camp_form_changes = false;
	$scope.show_asteric_tented_camp = '';
	...
	$scope.access_assessment_sheet = function(id,assessor_category){	
		$http({
			...
			if(($scope.assessment_id > 0) && ($scope.assessment_score != 'empty')){
				$scope.action = 'edit_record';					
				if( $scope.assessment_sheet_link == 'TH'){ $scope.fetch_tented_camp_items(assessment_id, assessor_category); $scope.assessment_score_tented_camp = $scope.assessment_score; $scope.open_assessment_sheet_for_tented_camp(); };
					if( $scope.assessment_sheet_link == 'VH'){ $scope.fetch_tented_camp_items(assessment_id, assessor_category); $scope.assessment_score_tented_camp = $scope.assessment_score; $scope.open_assessment_sheet_for_tented_camp(); };
			}else{
			$scope.action = 'insert_record';
				if( $scope.assessment_sheet_link == 'TH'){ $scope.assessment_score_tented_camp = 0; $scope.futa_radio_check_values_tented_camp(); $scope.open_assessment_sheet_for_tented_camp(); };
					if( $scope.assessment_sheet_link == 'VH'){ $scope.assessment_score_tented_camp = 0; $scope.futa_radio_check_values_tented_camp(); $scope.open_assessment_sheet_for_tented_camp(); };			
			};
...
//tented camp	 
app.tented_camp_ctrl($timeout, $scope, $http, focus);
//tented camp
app.tented_camp_ctrl($timeout, $scope, $http, focus);
//tented_camp
app.tented_camp_ctrl($timeout, $scope, $http, focus);
...
//////////////////////////// TALLYING SHEETS  ////////////////////////////////////////////////////////
	...
	$scope.access_tallying_sheet = function(id,assessor_category){	
		$http({
			...
			if(($scope.assessment_id > 0) && ($scope.assessment_score != 'empty')){
				//$scope.action = 'edit_record';					
				if( $scope.assessment_sheet_link == 'TH'){ $scope.fetch_tented_camp_items_for_tallying(assessment_id, assessor_category); };
				if( $scope.assessment_sheet_link == 'VH'){ $scope.fetch_tented_camp_items_for_tallying(assessment_id, assessor_category); };
			};
...
//tented camp Tallying	 
app.tented_camp_tallying_ctrl($timeout, $scope, $http, focus);
app.tented_camp_tallying_ctrl($timeout, $scope, $http, focus);
...
//ASSESSMENT SHEET - tented camp

$('.sum_tented_camp').on('click', function(){
    var sum_total_tented_camp = 0;
	var sum_checked_tented_camp = 0;
    $('.sum_tented_camp:checked').each(function(){
        sum_checked_tented_camp += Number(this.value);
			var sum_cm_tented_camp = 0;
			$('.sum_tented_camp_cm').each(function(){
			sum_cm_tented_camp += +$(this).val();
			});
		sum_total_tented_camp = sum_cm_tented_camp + sum_checked_tented_camp;			
		$('#total_tented_camp').text(sum_total_tented_camp);
		$('#assessment_score_tented_camp').val(sum_total_tented_camp);
		$('#assessment_score_tented_camp').trigger('input');
		//form changes
		$('#form_save_status_tented_camp').text('*');
		$('#show_asteric_tented_camp').val('*');
		$('#show_asteric_tented_camp').trigger('input');
    });	
});
$(document).on("click keyup", ".sum_tented_camp_cm", function() {
    var sum_total_tented_camp = 0;
	var sum_cm_tented_camp = 0;
	$(".sum_tented_camp_cm").each(function(){
		sum_cm_tented_camp += +$(this).val();
...
$('#reset_btn_tented_camp, #reset_btn_bottom_tented_camp').on('click', function(){
	$('.sum_tented_camp').prop('checked', false);
	var total_tented_camp = 0;
	$('#total_tented_camp').text(total_tented_camp);
	$('#assessment_score_tented_camp').val(total_tented_camp);
	$('#assessment_score_tented_camp').trigger('input');
	//form changes
	$('#form_save_status_tented_camp').text('*');
	$('#show_asteric_tented_camp').val('*');
