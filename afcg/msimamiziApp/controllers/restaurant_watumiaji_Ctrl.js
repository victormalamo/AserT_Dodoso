...
////////////////////// ESSENTIAL ITEMS ////////////////////////////////////// 
	...
	$scope.show_asteric = '';
	$scope.assessment_restaurant_form_changes = false;
	...
	$scope.access_assessment_sheet = function(id,assessor_category){	
		$http({
			...
			if(($scope.assessment_id > 0) && ($scope.assessment_score != 'empty')){
				$scope.action = 'edit_record';					
				if( $scope.assessment_sheet_link == 'TH'){ $scope.fetch_restaurant_items(assessment_id, assessor_category); $scope.assessment_score_restaurant = $scope.assessment_score; $scope.open_assessment_sheet_for_restaurant(); };
			...
			$scope.action = 'insert_record';
				if( $scope.assessment_sheet_link == 'TH'){ $scope.assessment_score_restaurant = 0; $scope.futa_radio_check_values_restaurant(); $scope.open_assessment_sheet_for_restaurant(); };
...
//Town Hotel	 
app.restaurant_ctrl($timeout, $scope, $http, focus);
//Vacation Hotel
...
//////////////////////////// TALLYING SHEETS  ////////////////////////////////////////////////////////
	...
	$scope.access_tallying_sheet = function(id,assessor_category){	
		$http({
			...
			if(($scope.assessment_id > 0) && ($scope.assessment_score != 'empty')){
				//$scope.action = 'edit_record';					
				if( $scope.assessment_sheet_link == 'TH'){ $scope.fetch_restaurant_items_for_tallying(assessment_id, assessor_category); };
...
//Town Hotel Tallying	 
app.restaurant_tallying_ctrl($timeout, $scope, $http, focus);
...
$('.sum_restaurant').on('click', function(){
    var sum_total_restaurant = 0;
	var sum_checked_restaurant = 0;
    $('.sum_restaurant:checked').each(function(){
        sum_checked_restaurant += Number(this.value);
			var sum_cm_restaurant = 0;
			$('.sum_restaurant_cm').each(function(){
			sum_cm_restaurant += +$(this).val();
			});
		sum_total_restaurant = sum_cm_restaurant + sum_checked_restaurant;			
		$('#total_restaurant').text(sum_total_restaurant);
		$('#assessment_score_restaurant').val(sum_total_restaurant);
		$('#assessment_score_restaurant').trigger('input');
		//form changes
		$('#form_save_status_restaurant').text('*');
		$('#show_asteric_restaurant').val('*');
		$('#show_asteric_restaurant').trigger('input');
    });	
});
$(document).on("click keyup", ".sum_restaurant_cm", function() {
    var sum_total_restaurant = 0;
	var sum_cm_restaurant = 0;
	$(".sum_restaurant_cm").each(function(){
		sum_cm_restaurant += +$(this).val();
...
$('#reset_btn_restaurant, #reset_btn_bottom_restaurant').on('click', function(){
	$('.sum_restaurant').prop('checked', false);
	var total_restaurant = 0;
	$('#total_restaurant').text(total_restaurant);
	$('#assessment_score_restaurant').val(total_restaurant);
	$('#assessment_score_restaurant').trigger('input');
	//form changes
	$('#form_save_status_restaurant').text('*');
	$('#show_asteric_restaurant').val('*');
