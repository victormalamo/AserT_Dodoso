app.vacation_hotel_ctrl = function($timeout, $scope, $http, focus){

	$scope.fetch_vacation_hotel_items = function(assessment_id, assessor_category){
			$http({
			method:"POST",
			url:"msimamiziApp/sajili_mtumiaji/fetch_vacation_hotel_items.php",
	...
	$scope.open_assessment_sheet_for_vacation_hotel = function(){
		var modal_popup = angular.element('#assessment_sheet_for_vacation_hotel_panel');
		modal_popup.modal('show');
	};
	$scope.close_assessment_sheet_for_vacation_hotel = function(){
		var modal_popup = angular.element('#assessment_sheet_for_vacation_hotel_panel');
		modal_popup.modal('hide');
	};
	$scope.check_for_assessment_form_changes_vacation_hotel = function(){
		if($scope.show_asteric_vacation_hotel == '*'){ $scope.assessment_vacation_hotel_form_changes = true; $scope.open_save_changes(); }else{ $scope.close_assessment_sheet_for_vacation_hotel(); };
	};
	$scope.save_and_exit_vacation_hotel= function(){
	$scope.from_save_vacation_hotel = 'save_and_exit';
	$scope.submit_assessment_vacation_hotel_form();
	};
	$scope.close_without_save_vacation_hotel= function(){
	$scope.close_assessment_sheet_for_vacation_hotel();
	$scope.close_save_changes();
	};
	
	$scope.submit_assessment_vacation_hotel_form = function(){
		if($scope.from_save_vacation_hotel == 'save_and_exit') { $scope.close_save_changes(); }else{ $scope.from_save_vacation_hotel = ''; };		
		...
		$http({
			method:"POST",
			url:"msimamiziApp/sajili_mtumiaji/insert_assessment_vacation_hotel.php",
			data:{					
				'facility_id':$scope.facility_id,
				'assessor_category':$scope.assessor_category,
				'assessment_id':$scope.assessment_id,
				'assessment_score_vacation_hotel':$scope.assessment_score_vacation_hotel,
				//LOCATION 1.0
				//1.1
				//1.1.1
			...
			$scope.fetchStatus();
				$timeout(function()
				{ 
					if($scope.from_save_vacation_hotel =='save_and_exit')
					{ 
					$scope.close_assessment_sheet_for_vacation_hotel(); $scope.close_funika_kurasa_with_status(); $scope.from_save_vacation_hotel = ''; 
					}
					else
					{ 
	...
	$scope.futa_radio_check_values_vacation_hotel = function(){
		//LOCATION 1.0
			//1.1
				//1.1.1
