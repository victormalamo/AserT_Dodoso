...
////////////////////// ESSENTIAL ITEMS ////////////////////////////////////// 
	...
	$scope.assessment_cottage_form_changes = false;
...
//cottage
app.cottage_ctrl($timeout, $scope, $http, focus);
//Motel
...
//ASSESSMENT SHEET - cottage

$('.sum_cottage').on('click', function(){
    var sum_total_cottage = 0;
	var sum_checked_cottage = 0;
    $('.sum_cottage:checked').each(function(){
        sum_checked_cottage += Number(this.value);
			var sum_cm_cottage = 0;
			$('.sum_cottage_cm').each(function(){
			sum_cm_cottage += +$(this).val();
			});
		sum_total_cottage = sum_cm_cottage + sum_checked_cottage;			
		$('#total_cottage').text(sum_total_cottage);
		$('#assessment_score_cottage').val(sum_total_cottage);
		$('#assessment_score_cottage').trigger('input');
		//form changes
		$('#form_save_status_cottage').text('*');
		$('#show_asteric_cottage').val('*');
		$('#show_asteric_cottage').trigger('input');
    });	
});
$(document).on("click keyup", ".sum_cottage_cm", function() {
    var sum_total_cottage = 0;
	var sum_cm_cottage = 0;
	$(".sum_cottage_cm").each(function(){
		sum_cm_cottage += +$(this).val();
...
$('#reset_btn_cottage, #reset_btn_bottom_cottage').on('click', function(){
	$('.sum_cottage').prop('checked', false);
	var total_cottage = 0;
	$('#total_cottage').text(total_cottage);
	$('#assessment_score_cottage').val(total_cottage);
	$('#assessment_score_cottage').trigger('input');
	//form changes
	$('#form_save_status_cottage').text('*');
	$('#show_asteric_cottage').val('*');
