...
include('kagua_fields_tupu_update_hod_u_yaliyokataliwa.php');
include('aina_ya_kibali_hod_u.php'); ///// JADILI HILI JUU YA UDHARURA UNAOWEZA KUJITOKEZA TAREHE ZINAPOBADILISHWA
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "yaliyokataliwa_list_updateForm")) {
  $updateSQL = sprintf("UPDATE vibali SET ruti_ID=%s, catID=%s, regID=%s, lgaID=%s, driver_username=%s, jina_la_halmashauri=%s, distance=%s, duration=%s, sDate=%s, eDate=%s, duration_safarini=%s, usafiriID=%s, madhumuniID=%s, muhtasari=%s, mlipajiID=%s, wengine=%s, last_edited_hodu=%s, aina_ya_kibali=%s, sababu_ya_aina=%s  WHERE id=%s",
					   ...
					   GetSQLValueString($_POST['driver_username'], "text"),
					   ...
					   GetSQLValueString($_POST['last_edited_hod_u'], "text"),
  ...
  $Result1 = mysqli_query($conntscpME, $updateSQL) or die(mysqli_error());
	...
	echo "window.location.href='yaliyokataliwa_tazama.php?id=$kibali_id&imetoka=badili_maombi_hod_u';";
	echo "</script>";
	
} else { ///verification code imekataa
	header("Location: yaliyokataliwa_list_update.php?ruti_ID=$ruti_ID&jina_la_halmashauri=$jina_la_halmashauri&distance=$distance&duration=$duration&catID=$catID&regID=$regID&lgaID=$lgaID&driver_username=$driver_username&sDate=$sDate&eDate=$eDate&duration_safarini=$duration_safarini&usafiriID=$usafiriID&madhumuniID=$madhumuniID&muhtasari=$muhtasari&mlipajiID=$mlipajiID&wengine=$wengine&id=$kibali_id&wrong_code=true");
