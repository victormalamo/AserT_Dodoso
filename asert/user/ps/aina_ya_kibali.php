...
//Jumla ya idadi ya siku zilizoombwa kwa vibali vilivyokwisha pitishwa na ps (Tazama kama safari imekamilika na idadi ya siku???)
$financial_year_c = "FY_2018_2019"; //Mwaka husika
$general_status_c = "kimepitishwa_ps"; //idadi ya siku kwa vibali vilivyokwisha pitishwa
$colname1_rs_siku_za_mwaka = "1";
if (isset($_SESSION['username'])) {
  $colname1_rs_siku_za_mwaka = (get_magic_quotes_gpc()) ? $_SESSION['username'] : addslashes($_SESSION['username']);
}

$query_rs_siku_za_mwaka = sprintf("SELECT SUM(duration) AS siku_za_mwaka FROM vibali WHERE (((`username` = '%s' ) AND (`financial_year` = '%s')) AND (`general_status` = '%s'))", $colname1_rs_siku_za_mwaka, $financial_year_c, $general_status_c );
...
$query_rs_siku_vibali_vipya = sprintf("SELECT SUM(duration) AS siku_vibali_vipya FROM vibali WHERE ((`id` != '%s') AND (`username` = '%s' ) AND (`financial_year` = '%s') AND (`general_status` = '%s'))", $id, $colname1_rs_siku_vibali_vipya, $financial_year_c, $general_status_c );
